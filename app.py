...
capacity nodes) against a physical substations layer to identify where solar + storage
can be sited most easily.

Core capabilities:
  1. Screen any state by voltage class and headroom threshold, returning bus names,
     coordinates and every congestion parameter, with an on-map view and CSV export.
  2. A proximity tool: enter coordinates or an address to return the nearest
     medium-voltage nodes (with headroom) and the nearest physical substations.
  3. A data-grounded assistant that answers questions about the tables in plain English.

The app is driven entirely by the column registry in data_loader.py, so any additional
ISO/RTO export in the same schema (e.g. NYISO) works with no code changes.
"""
...
def center_zoom(df):
    """Center + a sensible zoom computed from the data's bounding box.
    Prevents the map from defaulting to (0,0) when no center is supplied."""
    lat, lon = df[dl.LAT_COL], df[dl.LON_COL]
    if len(df) == 0 or lat.isna().all() or lon.isna().all():
        return {"lat": 43.0, "lon": -71.5}, 5.0
    center = {"lat": float(lat.mean()), "lon": float(lon.mean())}
    span = max(float(lat.max() - lat.min()), float(lon.max() - lon.min()), 0.05)
    zoom = float(np.clip(8.5 - np.log2(span), 3.0, 12.0))
    return center, zoom
...
with tab_map:
    ...
    st.markdown('<div class="sub">Every physical & congestion parameter is a filter. '
                'Use a preset, or build your own screen.</div>',
                unsafe_allow_html=True)

    # Preset buttons
    pc1, pc2, pc3, _ = st.columns([1.1, 1.1, 1, 3])
    preset = st.session_state.get("preset", {})
    if pc1.button("⭐ MA · 69/115 kV · >600 MW headroom", use_container_width=True):
...
with tab_lookup:
    ...
    st.markdown('<div class="sub">Enter coordinates or an address; '
    ...
    if lat is not None and lon is not None:
        ...
        # nearest physical substations too (names + coordinates)
        if not subs.empty:
            sp = subs.copy()
...
# ============================================================ TAB 4: DICTIONARY
with tab_about:
    # ── how the headline numbers relate (with a live, data-driven worked example)
    st.markdown('<div class="sect">How the numbers relate</div>', unsafe_allow_html=True)

    # pick a real, illustrative bus: high headroom with |shift factor| meaningfully < 1
    ex = buses[(buses["Shift Factor (Number)"].abs().between(0.2, 0.95)) &
               (buses[dl.HEADROOM_COL] > 0)]
    ex = ex.sort_values(dl.HEADROOM_COL, ascending=False)
    if len(ex):
        r = ex.iloc[0]
        H = float(r[dl.HEADROOM_COL]); SF = float(r["Shift Factor (Number)"])
        Cimp = H / abs(SF); Crep = float(r[dl.CAPACITY_COL])
        ex_bus = str(r["Bus Name"]); ex_v = f"{r[dl.VOLTAGE_COL]:g}"
        ex_loc = f"{r.get('County','?')} County, {r.get('State','?')}"
        ex_con = str(r.get("Primary Limiting Constraint", "its binding constraint"))
        example_md = (
            f"**Worked example — {ex_bus} ({ex_v} kV, {ex_loc}).** "
            f"Its first binding constraint is `{ex_con}`. "
            f"Headroom **H = {H:,.0f} MW** and shift factor **SF = {SF:.2f}**. "
            f"A project injecting *P* MW here adds `|SF|·P = {abs(SF):.2f}·P` MW of flow to that "
            f"constraint, so the largest injection before it binds is "
            f"**P = H ÷ |SF| = {H:,.0f} ÷ {abs(SF):.2f} ≈ {Cimp:,.0f} MW** — which matches the "
            f"reported interconnection capacity of **{Crep:,.0f} MW**. In plain terms: you could "
            f"site roughly a **{Crep:,.0f} MW** project at this bus before this constraint is the "
            f"limiting factor.")
    else:
        example_md = "_No bus with an illustrative shift factor is available in the current data._"

    st.markdown(f"""<div class="card">
The three headline figures are tied together by the <b>shift factor</b>:

<ul style="margin:8px 0 4px 18px;line-height:1.7">
<li><b>Shift factor (SF)</b> — the fraction of power injected at a bus that shows up as
extra flow on its binding constraint. &nbsp;<code>ΔFlow_constraint = SF · P_inject</code>.
Range roughly −1…+1; the sign is just direction, what matters for siting is <b>|SF|</b>.</li>
<li><b>Headroom (H, MW)</b> — how much more flow the binding constraint can carry, under the
studied N-1 contingency, before it reaches its limit.</li>
<li><b>Interconnection capacity (C, MW)</b> — the largest injection at the bus before that
constraint is exhausted. Set <code>SF · P = H</code> and solve for P:
&nbsp;<b><code>C = H ÷ |SF|</code></b>. &nbsp;(This identity reproduces the reported capacity
for ~91% of nodes in this dataset.)</li>
<li><b>Constraint loading (%)</b> — how loaded the limiting element already is
(<code>base flow ÷ rating</code>). Near or above 100%, or <b>Already overloaded = true</b>,
means little or no headroom regardless of SF.</li>
<li><b>Upgrade risk</b> — how hard/expensive it would be to relieve the constraint and unlock
capacity beyond C.</li>
</ul>

<div style="background:{PAPER};border-radius:10px;padding:12px 14px;margin-top:10px;font-size:13.5px">
{example_md}
</div>

<div style="margin-top:10px;font-size:13.5px">
<b>Screening rule of thumb:</b> high <b>headroom</b> + low <b>|shift factor|</b> + low
<b>constraint loading</b> + low <b>upgrade risk</b> = the most siteable node. A large |SF|
means your MW load the constraint harder, so <i>less</i> capacity fits for the same headroom;
a small |SF| means <i>more</i> fits.
</div>
</div>""", unsafe_allow_html=True)

    st.markdown('<div class="sect">What each column means</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub">The buses table is the Orennia Greenfield Interconnection '
                'Capacity export. Headroom, capacity, loading, shift factor and risk are the '
                'siting parameters. This export covers ISO-NE; load an export from another '
                'ISO/RTO in the sidebar to screen that region with no code changes.</div>',
                unsafe_allow_html=True)
    dd = pd.DataFrame(
